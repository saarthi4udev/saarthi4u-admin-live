...
export default function FacultyList({ facultys, handleDelete }: any) {
    const [search, setSearch] = useState("");

    const filteredFaculty = facultys.filter((c: any) =>
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gray-2 dark:bg-meta-4 text-left">
                                ...
                                <th className="py-3 px-4 font-medium text-black dark:text-white">Designation</th>
                                <th className="py-3 px-4 font-medium text-black dark:text-white">Qualification</th>
                                <th className="py-3 px-4 font-medium text-black dark:text-white">Experience</th>
                        ...
                        <tbody>
                            {filteredFaculty.map((faculty: any, index: number) => (
                                ...
                                >
                                    ...
                                    <td className="py-3 px-4 text-black dark:text-white">{faculty.name || "No Data"}</td>
                                    <td className="py-3 px-4 text-black dark:text-white">{faculty.designation || "No Data"}</td>
                                    <td className="py-3 px-4 text-black dark:text-white">{faculty.qualification || "No Data"}</td>
                                    <td className="py-3 px-4 text-black dark:text-white">{faculty.experience || "No Data"}</td>
