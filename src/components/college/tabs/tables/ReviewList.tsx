...
export default function ReviewList({ reviews, handleDelete }: any) {
    const [search, setSearch] = useState("");

    const filteredReviews = reviews.filter((c: any) =>
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="bg-gray-2 dark:bg-meta-4 text-left">
                                ...
                                <th className="py-3 px-4 font-medium text-black dark:text-white">Username</th>
                                <th className="py-3 px-4 font-medium text-black dark:text-white">Rating</th>
                                <th className="py-3 px-4 font-medium text-black dark:text-white">Comment</th>
                        ...
                        <tbody>
                            {filteredReviews.map((review: any, index: number) => (
                                ...
                                >
                                    ...
                                    <td className="py-3 px-4 text-black dark:text-white">{review.username || "No Data"}</td>
                                    <td className="py-3 px-4 text-black dark:text-white">{review.rating || "No Data"}</td>
                                    <td className="py-3 px-4 text-black dark:text-white">{review.comment || "No Data"}</td>
                                    {/* Actions */}
