...
const SignIn = () => {
  ...
  const authHighlights = [
    ...
    'Activity-ready operations panel',
  ];
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Optionally, display an error toast or show an error message to the user
      toast.error(
  ...
  return loading ? (
    ...
    <div className="relative min-h-screen overflow-x-hidden bg-whiten dark:bg-boxdark-2 lg:h-screen lg:overflow-hidden">
      <div className="auth-grid-overlay pointer-events-none absolute inset-0" />
      <div className="auth-float-slow pointer-events-none absolute -left-24 -top-24 h-80 w-80 rounded-full bg-primary/25 blur-3xl" />
      <div className="auth-float-delay pointer-events-none absolute -right-20 top-1/3 h-96 w-96 rounded-full bg-meta-5/20 blur-3xl" />
      <div className="auth-float-delay pointer-events-none absolute -bottom-24 right-1/5 h-72 w-72 rounded-full bg-secondary/30 blur-3xl" />

      <div className="auth-login-shell mx-auto flex min-h-screen w-full max-w-[1700px] items-center px-3 py-4 sm:px-6 lg:h-full lg:min-h-0 lg:py-3">
        <div className="auth-main-card auth-card-pop w-full overflow-hidden rounded-2xl border border-stroke/80 bg-white/90 shadow-6 backdrop-blur-sm dark:border-strokedark dark:bg-boxdark/90">
          <div className="flex flex-col xl:h-full xl:flex-row xl:items-center">
            <div className="hidden w-full xl:block xl:w-1/2">
              <div className="auth-left-panel relative px-8 py-8 text-center 2xl:px-20">
            ...
            <div className="mx-auto mb-5 max-w-[460px]">
              <h3 className="auth-fade-up mb-2 text-title-sm font-bold text-black dark:text-white">
                Smart, secure and fast admin access
              </h3>
              <p className="auth-fade-up text-body dark:text-bodydark">
                Manage users, categories and college data with a premium admin experience.
              </p>
              <div className="auth-highlights mt-4 grid gap-2.5 text-left">
                {authHighlights.map((item, index) => (
                  <div
                    key={item}
                    className={`auth-fade-up flex items-center gap-2.5 rounded-lg border border-stroke/80 bg-white/60 px-4 py-3 text-sm font-medium text-black shadow-1 dark:border-strokedark dark:bg-boxdark-2 dark:text-bodydark1 ${index === 0 ? 'auth-stagger-1' : index === 1 ? 'auth-stagger-2' : 'auth-stagger-3'}`}
                  >
            ...
            <span className="auth-float-delay mt-5 inline-block">
              <svg
                className="auth-hero-illustration h-[180px] w-[180px] 2xl:h-[220px] 2xl:w-[220px]"
                width="350"
                height="350"
                viewBox="0 0 350 350"
                fill="none"
        ...
        <div className="w-full xl:w-1/2 xl:border-l-2 xl:border-stroke/70 xl:dark:border-strokedark">
          <div className="auth-right-wrap w-full p-5 sm:p-8 xl:p-10">
            <div className="auth-form-card auth-fade-up mx-auto max-w-[560px] rounded-2xl border border-stroke/80 bg-white/85 p-5 shadow-6 backdrop-blur-sm dark:border-strokedark dark:bg-boxdark-2/70 sm:p-6">
              <div className="mb-4 xl:hidden">
                <Link className="inline-block" to="/">
                  <img src={Logo} alt="Logo" style={{ width: '180px' }} />
                </Link>
              </div>
              <p className="mb-3 inline-flex rounded-full bg-primary/10 px-4 py-1 text-sm font-medium text-primary dark:bg-primary/20">
                Welcome Back
              </p>
              <h2 className="mb-2 text-title-md font-bold text-black dark:text-white sm:text-title-lg">
                Sign In to Saarthi4u-Admin Panel
              </h2>
              <p className="mb-4 text-base text-body dark:text-bodydark">
                Securely access your dashboard and manage platform operations.
              </p>

              <div className="mb-5 flex flex-wrap gap-2">
                <span className="rounded-full bg-meta-2 px-3 py-1 text-xs font-medium text-black dark:bg-meta-4 dark:text-bodydark1">
                  Security First
                </span>
                <span className="rounded-full bg-meta-2 px-3 py-1 text-xs font-medium text-black dark:bg-meta-4 dark:text-bodydark1">
                  Admin Control
              ...
              <div className="mb-4">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full rounded-lg border border-stroke bg-transparent py-3.5 pl-5 pr-10 text-black shadow-1 outline-none transition-all duration-300 focus:border-primary focus:shadow-3 focus-visible:shadow-none hover:border-primary/40 dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary"
                  />
              ...
              <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
                <label className="inline-flex items-center gap-2 text-sm text-body dark:text-bodydark">
                  <input
                    type="checkbox"
              ...
              <div className="mb-4">
                <button
                  type="submit"
                  className="auth-gradient-shift auth-button-click w-full rounded-lg border border-primary bg-gradient-to-r from-primary via-meta-5 to-primary p-3.5 text-lg font-semibold text-white shadow-4 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-6 active:translate-y-0"
                >
                  Sign In
                </button>
              </div>
              ...
              <p className="text-center text-sm text-body dark:text-bodydark">
                Protected login with secure admin session handling.
              </p>
            </form>
            </div>
          </div>
